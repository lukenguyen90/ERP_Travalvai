(function() {
    ...
    angular
        .module("order.ctrls")
        .controller('EditNameNumberCtrl', editNameNumberCtrl);

    function editNameNumberCtrl($scope, $filter, $http, $compile, $window, $timeout, $controller, models, productService) {
        var vm = this;
        ...
        var templates = {
            manual: "../../views/order/client/product/views/edit/size-detail-manual.view.html",
            csv: "../../views/order/client/product/views/size-detail-csv.view.html"
        }
        ...
        function initSizeNumer() {
            ...
            vm.tplInput = getTemplate(vm.inputType.manual)
        ...
        function initListSize() {
            ...
            angular.forEach(vm.order.orderItems, function(sizeDetail, key) {
                if (sizeDetail.quantity.toString().length > 0) {
                    var size = angular.copy(sizeDetail);
                    vm.listSize.push(size);
                    units += parseInt(sizeDetail.quantity);
                }
            });

            vm.product.units = units;
            vm.totalSizeQuantity = units;
        ...
        function hasSizeDetail() {
            ...
            if(vm.size !== null){
                if( (typeof(vm.size.name) === 'undefined' && typeof(vm.size.number) === 'undefined') 
                    || vm.size.name == ''
                    || vm.size.number == ''
                    || typeof(vm.size.code) === 'undefined'
                    || typeof(vm.size.quantity) === 'undefined' 
                    || typeof(vm.size.quantity) === ''
                    || vm.size.code.length === 0)
                    return true;
            }
            return false;
        }
        ...
        function upateSizeSummary() {
            ...
            vm.totalSizeQuantity = units;
            vm.product.price.grandTotal = $scope.$parent.calculateGrandTotal(units);
            vm.order.orderItems = angular.copy(vm.listSize);
        ...
        function updateSizeFromExcel() {
            if (vm.csv.result !== null && vm.csv.result.length > 0) {
                ...
                $scope.$parent.$parent.vmOrderDetail.arrErrCSV = arrayErrImport;

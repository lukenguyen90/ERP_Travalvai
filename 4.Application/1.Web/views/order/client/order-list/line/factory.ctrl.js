(function() {
    ...
    function lineFactoryCtrl($q, $scope, $filter, $http, $compile, DTOptionsBuilder, DTColumnBuilder, $window, $timeout, $controller, $resource, orderListService) {
        var vm = this;
        ...
        function loadDatatable() {
            ...
            vm.columns = [
                ...
                DTColumnBuilder.newColumn(null).withTitle('').notSortable().renderWith(vm.renderDetail).withOption("width", "2%").withClass("text-center detail"), //15
        ...
        function renderCustomer(data, type, full, meta) {
            ...
            return '<span style="minheight:50px;">' + data.id_customer + " - " + data.cs_name +
                '</span><hr><span style="minheight:50px;">' +
                data.ag_code + " - " + data.ag_description + '</span><hr><span style="minheight:50px;">' +
                data.zone_code + " - " + data.z_description + '</span>';
        }
        ...
        function renderpendent(data, type, full, meta) {
            return '<span style="minheight:50px;">' + full.ord_fty_pendent + '</span><hr><span style="minheight:50px;">' + full.ord_zone_pendent + '</span><hr><span style="minheight:50px;">' + full.ord_ag_pendent + '</span>';

        }

        function rendervalues(data, type, full, meta) {
            return '<span style="minheight:50px;">' +
                formatNumberThousand(data.ord_fty_value, 2) + '-' + data.ord_plf + '</span><hr><span style="minheight:50px;">' +

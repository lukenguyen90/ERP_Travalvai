(function() {
    ...
    angular
        .module('order.ctrls')
        .controller("SizeAgentCtrl", sizeAgentCtrl);

    function sizeAgentCtrl($scope, $filter, $http, $compile, DTOptionsBuilder, DTColumnBuilder, $window, $timeout, $controller, $resource, orderDetailService) {
        var vm = this;
        ...
        function renderFooter(tfoot, row, data, start, end, display) {
            ...
            footerTpl = '<tr><td colspan="27"></td></tr>' +
                ...
                '<td></td>' +
                '<td class="text-right">' + formatThousand(zoneTotalPrice, 2) + ' - ' + zoneCurrency + '<hr>' + formatThousand(agentTotalPrice, 2) + ' - ' + agentCurrency +
                '</td>' +
                '<td colspan="2"></td>' +
                '</tr>';

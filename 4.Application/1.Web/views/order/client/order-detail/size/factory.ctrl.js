(function() {
    ...
    angular
        .module('order.ctrls')
        .controller("SizeFactoryCtrl", sizeFactoryCtrl);

    function sizeFactoryCtrl($scope, $filter, $http, $compile, DTOptionsBuilder, DTColumnBuilder, $window, $timeout, $controller, $resource, orderDetailService) {
        var vm = this;
        ...
        function initDatatable() {
            ...
            vm.columns = [
                ...
                DTColumnBuilder.newColumn('id_product').withTitle('PRODUCT'), //0 prd_sketch
                ...
                DTColumnBuilder.newColumn('priceList').withTitle('priceList').withOption("visible", false) //47
            ]
        ...
        function renderFooter(tfoot, row, data, start, end, display) {
            ...
            var sum5XS = utils().dataTable().getColumnSummary(api, 35);
            ...
            footerTpl = '<tr><td colspan="27"></td></tr>' +
                ...
                '<td></td>' +
                '<td class="text-right">' + formatThousand(ftyTotalPrice, 2) + ' - ' + ftyCurrency +
                '<hr>' + formatThousand(zoneTotalPrice, 2) + ' - ' + zoneCurrency + '<hr>' + formatThousand(agentTotalPrice, 2) + ' - ' + agentCurrency +
                '</td>' +
                '<td colspan="2"></td>' +
                '</tr>';

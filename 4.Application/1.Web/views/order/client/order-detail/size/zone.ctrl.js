(function() {
    ...
    angular
        .module('order.ctrls')
        .controller("SizeZoneCtrl", sizeZoneCtrl);

    function sizeZoneCtrl($scope, $filter, $http, $compile, DTOptionsBuilder, DTColumnBuilder, $window, $timeout, $controller, $resource, orderDetailService) {
        var vm = this;
        ...
        function renderPrice(data, type, full, meta) {
            return '<span style="minheight:50px;">' +
                formatNumberThousand(data.price_factory) + ' - ' + data.ord_plf_currency + '</span><hr><span style="minheight:50px;">' +
                formatNumberThousand(data.price_zone) + ' - ' + data.ord_plz_currency + '</span><hr><span style="minheight:50px;">' +
                formatNumberThousand(data.price_agent) + ' - ' + data.ord_plz_currency + '</span>';

(function() {
    ...
    angular
        .module('order.ctrls')
        .controller("LineFactoryCtrl", lineFactoryCtrl);

    function lineFactoryCtrl($scope, $filter, $http, $compile, DTOptionsBuilder, DTColumnBuilder, $window, $timeout, $controller, $resource, orderDetailService) {
        var vm = this;
        ...
        function renderFooter(tfoot, row, data, start, end, display) {
            ...
            footerTpl = '<tr><td colspan="16"></td></tr>' +
                ...
                '<td colspan="2"></td>' +
                '<td class="text-right">' + formatNumberThousand(ftyTotalPrice, 2) + (ftyCurrency != " " ? (' - ' + ftyCurrency) :' - ')  +
                '<hr>' + formatNumberThousand(zoneTotalPrice, 2) + (zoneCurrency != " " ? (' - ' + zoneCurrency) :' - ') +  '<hr>' + formatNumberThousand(agentTotalPrice, 2) + (agentCurrency != " " ? (' - ' + agentCurrency) :' - ') +
                '</td>' +
                '<td colspan="2"></td>' +
                '</tr>';
        ...
        function renderDelivered(data, type, full, meta) {
            return '<span style="minheight:50px;">' + data.ordd_del_fty + '</span><hr><span style="minheight:50px;">' + data.ordd_del_zone + '</span><hr><span style="minheight:50px;">' + data.ordd_del_ag + '</span>';

        }

        function renderPrice(data, type, full, meta) {
            return '<span style="minheight:50px;">' +
                formatNumberThousand(data.price_factory, 2) + ' - ' + data.ord_plf_currency + '</span><hr><span style="minheight:50px;">' +
                formatNumberThousand(data.price_zone, 2) + ' - ' + data.ord_plz_currency + '</span><hr><span style="minheight:50px;">' +
                formatNumberThousand(data.price_agent, 2) + ' - ' + data.ord_plz_currency + '</span>';

        }

        function renderTotalPrice(data, type, full, meta) {
            return '<span style="minheight:50px;">' +
                formatNumberThousand(data.ordd_fty_tot, 2) + ' - ' + data.ord_plf_currency + '</span><hr><span style="minheight:50px;">' +
                formatNumberThousand(data.ordd_zone_tot, 2) + ' - ' + data.ord_plz_currency + '</span><hr><span style="minheight:50px;">' +
                formatNumberThousand(data.ordd_ag_tot, 2) + ' - ' + data.ord_plz_currency + '</span>';

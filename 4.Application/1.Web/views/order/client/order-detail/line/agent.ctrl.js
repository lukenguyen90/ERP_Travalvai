(function() {
    ...
    function lineAgentCtrl($scope, $filter, $http, $compile, DTOptionsBuilder, DTColumnBuilder, $window, $timeout, $controller, $resource, orderDetailService) {
        var vm = this;
        ...
        function loadDatatable() {
            ...
            // if($scope.vmLine.isUpdate == false){
            //     vm.columns[14].visible = false;
            //     window.setTimeout(function() {
            //         console.log($("#tbl-product-list-line thead tr:last-child th:last-child"));
            //         $("#tbl-product-list-line thead tr:last-child th:last-child").remove();
            //     }, 100);
            // }       
        }
        ...
        function renderFooter(tfoot, row, data, start, end, display) {
            ...
            footerTpl = '<tr><td colspan="16"></td></tr>' +
                ...
                '<td colspan="2"></td>' +
                '<td class="text-right">'+formatNumberThousand(zoneTotalPrice, 2) + (zoneCurrency != " " ? (' - ' + zoneCurrency) :' - ') + '<hr>' + formatNumberThousand(agentTotalPrice, 2) + (agentCurrency != " " ? (' - ' + agentCurrency) :' - ') +
                '</td>' +
                '<td colspan="2"></td>' +
                '</tr>';
        ...
        function renderDelivered(data, type, full, meta) {
            return '<span style="minheight:50px;">' + data.ordd_del_zone + '</span><hr><span style="minheight:50px;">' + data.ordd_del_ag + '</span>';

(function(){
  	...
  	function BindAngularDirectiveCtrl($scope ,$filter, $http ,$compile, DTOptionsBuilder, DTColumnBuilder)
  	{
    	...
    	var originalPLZD   = angular.copy(vm.plz_dets);
     	...
     	vm.dtColumns_detail  = [
     		DTColumnBuilder.newColumn('COST_CODE').withTitle('CODE').withOption("width","10%"),
			DTColumnBuilder.newColumn('CD_DESCRIPTION').withTitle('DESCRIPTION'),
     		DTColumnBuilder.newColumn('CV_VERSION').withTitle('No.').withClass("text-center").withOption("width","6%"),
			DTColumnBuilder.newColumn('CVD_DESCRIPTION').withTitle('DESCRIPTION').withOption("width","18%"),
			...
			DTColumnBuilder.newColumn('PLZD_MARGIN_2').withTitle('AGENT MARGIN').renderWith(formatPercent).withClass("text-right th-align-center").withOption('width', "4%")
      	];
